 ...
 *
 * <p>We mock all dependencies (repositories, mappers, parserService) and test ONLY business logic
 * of PlaylistServiceImpl.
 */
...
class PlaylistServiceImplTest {
  ...
  @Mock private PlaylistTrackMapper playlistTrackMapper;
  ...
  void getById_shouldReturnDto_whenPlaylistExists() {
    ...
    PlaylistDto dto =
        PlaylistDto.builder().id(id).soundcloudUrl("url").title("title").totalTracks(0).build();
  ...
  void getById_shouldThrow_whenPlaylistNotFound() {
    ...
    NotFoundException ex = assertThrows(NotFoundException.class, () -> playlistService.getById(id));
  ...
  void getAll_shouldReturnMappedDtos() {
    ...
    PlaylistDto dto1 =
        PlaylistDto.builder().id(1L).soundcloudUrl("u1").title("t1").totalTracks(1).build();
    PlaylistDto dto2 =
        PlaylistDto.builder().id(2L).soundcloudUrl("u2").title("t2").totalTracks(2).build();
  ...
  void delete_shouldDeleteFutureScheduleAndPlaylist_whenExists() {
    ...
    verify(scheduleEntryRepository)
        .deleteByPlaylistIdAndStartTimeAfter(eq(playlistId), any(OffsetDateTime.class));
  ...
  void delete_shouldThrow_whenPlaylistNotFound() {
    ...
    NotFoundException ex =
        assertThrows(NotFoundException.class, () -> playlistService.delete(playlistId));

    assertTrue(ex.getMessage().contains("Playlist not found"));
    verify(scheduleEntryRepository, never()).deleteByPlaylistIdAndStartTimeAfter(anyLong(), any());
  ...
  void getTracks_shouldThrow_whenPlaylistNotFound() {
    ...
    NotFoundException ex =
        assertThrows(NotFoundException.class, () -> playlistService.getTracks(playlistId));
  ...
  void addTrack_shouldAppendTrackAtNextPosition() {
    ...
    PlaylistTrackDto dto = PlaylistTrackDto.builder().id(100L).position(3).build();
  ...
  void addTrack_shouldThrow_whenPlaylistNotFound() {
    ...
    NotFoundException ex =
        assertThrows(NotFoundException.class, () -> playlistService.addTrack(1L, 2L));
  ...
  void importPlaylistFromUrl_shouldThrow_whenPlaylistAlreadyExists() {
    ...
    when(playlistRepository.findByScPlaylistUrl(url)).thenReturn(Optional.of(new Playlist()));

    IllegalArgumentException ex =
        assertThrows(
            IllegalArgumentException.class, () -> playlistService.importPlaylistFromUrl(url));
