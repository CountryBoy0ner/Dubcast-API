...
class SoundCloudRadioProgrammingServiceImplTest {
    ...
    @Mock
    private AutoFillStrategy autoFillStrategy;

    @InjectMocks
    private SoundCloudRadioProgrammingServiceImpl service;
    ...
    void appendPlaylistToSchedule_shouldReturnResponseWithPlaylistAndEntries() {
        ...
        ScheduleEntryDto entryDto = ScheduleEntryDto.builder()
                .id(100L)
                .track(trackDto)
                .startTime(start)
                .endTime(end)
    ...
    void appendPlaylistToSchedule_shouldThrow_whenPlaylistNotFound() {
        ...
        assertThrows(NotFoundException.class,
                () -> service.appendPlaylistToSchedule(playlistId));
    ...
    void getDaySchedule_shouldMapScheduleEntriesToAdminSlots_usingPlaylistCache() {
        ...
        ScheduleEntryDto seDto = ScheduleEntryDto.builder()
                .id(100L)
                .track(trackDto)
                .startTime(start)
                .endTime(end)
        ...
        Page<ScheduleEntryDto> sePage = new PageImpl<>(
                List.of(seDto),
                pageable,
                1
        );
        ...
        when(playlistRepository.findAllById(Set.of(playlistId)))
    ...
    void insertTrackIntoDay_shouldMapToAdminSlotDto_withoutPlaylistNameLookupWhenPlaylistNull() {
        ...
        when(scheduleEntryService.insertTrackIntoDay(date, trackId, position))
                .thenReturn(seDto);

        AdminScheduleSlotDto result =
                service.insertTrackIntoDay(date, trackId, position);
    ...
    void changeTrackInSlot_shouldMapToAdminSlotDto() {
        ...
        when(scheduleEntryService.changeTrackInSlot(slotId, newTrackId))
                .thenReturn(seDto);

        AdminScheduleSlotDto result =
                service.changeTrackInSlot(slotId, newTrackId);
    ...
    void ensureAutofillIfNeeded_shouldAppendTrackAndReturnTrue_whenTrackChosen() {
        ...
        when(scheduleEntryService.getCurrent(now)).thenReturn(Optional.empty());
        when(scheduleEntryService.getNext(now)).thenReturn(Optional.empty());
        when(autoFillStrategy.chooseTrackIdForAutofill(now))
                .thenReturn(Optional.of(trackId));
