...
@Tag(
    name = "Playlists",
    description =
        "Endpoints for managing radio playlists and importing them from external sources.")
public class PlaylistController {
  ...
  public ResponseEntity<PlaylistDto> getById(
      @Parameter(description = "ID of the playlist", example = "1") @PathVariable Long id) {
  ...
  @Operation(
      ...
      description =
          "Deletes a playlist by its identifier. This operation is typically available only for admins.")
  @ApiResponses({
    @ApiResponse(
        responseCode = "204",
  ...
  public ResponseEntity<Void> delete(
      @Parameter(description = "ID of the playlist to delete", example = "1") @PathVariable
          Long id) {
